...
lysate_mat <- npx_lod %>%
  ...
  dplyr::mutate(
    avg_lysate_NPX = rowMeans(dplyr::across(all_of(lysates)), na.rm = TRUE)
  ) %>%
  # set column order: Assay, avg, then lysates in desired order
  dplyr::select(Assay, avg_lysate_NPX, dplyr::all_of(lysates)) %>%
  dplyr::arrange(desc(avg_lysate_NPX))

lysate_mat %>% dplyr::glimpse()

write.csv(
  lysate_mat,
  file = here::here(output_dir_data, "lysate_mat.csv"),
  quote = FALSE,
  row.names = FALSE
)
```
...
sup_medium_mat <- npx_lod %>%
  dplyr::filter(
    SampleID %in% sup_medium,
    Matrix %in% c("Medium","Supernatant")
  ) %>%
  dplyr::select(Assay, SampleID, NPX) %>%
  distinct() %>%
  tidyr::pivot_wider(
  ...
  dplyr::mutate(
    avg_sup_medium_NPX = rowMeans(dplyr::across(all_of(sup_medium)), na.rm = TRUE)
  ) %>%
  dplyr::select(Assay, avg_sup_medium_NPX, dplyr::all_of(sup_medium)) %>%  # enforce order
  dplyr::arrange(desc(avg_sup_medium_NPX))

sup_medium_mat %>% dplyr::glimpse()

write.csv(
  sup_medium_mat,
  file = here::here(output_dir_data, "sup_medium_mat.csv"),
  quote = FALSE,
  row.names = FALSE
)
```
